#!/usr/bin/env bash
#
# mysql-backup.sh — mysqldump a database, gzip-compress it, optionally upload
# to S3 or a local/mounted directory, verify the result, and print a summary.
#
# Usage: ./mysql-backup.sh --database <name> [options]
...
Dump a MySQL/MariaDB database with mysqldump, gzip-compress it to
--output-dir, optionally upload it, verify the archive, and print a summary.
...
Examples:
  ${SCRIPT_NAME} --database app --user backup --password-env MYSQL_PWD
  ${SCRIPT_NAME} --host db.internal --database app --user backup \\
      --password-env MYSQL_PWD --upload-s3 my-backups-bucket
EOF
}

log()  { printf '[mysql-backup] %s\n' "$*"; }
err()  { printf '[mysql-backup] ERROR: %s\n' "$*" >&2; }
...
[ -n "$PORT" ] && MYSQLDUMP_ARGS+=(--port="$PORT")
[ -n "$USER" ] && MYSQLDUMP_ARGS+=(--user="$USER")
MYSQLDUMP_ARGS+=("$DATABASE")
...
if [ -n "$PASSWORD" ]; then
  MYSQL_PWD="$PASSWORD" mysqldump "${MYSQLDUMP_ARGS[@]}" | gzip > "$OUTPUT_FILE"
else
  mysqldump "${MYSQLDUMP_ARGS[@]}" | gzip > "$OUTPUT_FILE"
fi
