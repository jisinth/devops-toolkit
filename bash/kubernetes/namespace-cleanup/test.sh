#!/usr/bin/env bash
#
# test.sh — basic checks for namespace-cleanup.sh. Run: ./test.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT="${SCRIPT_DIR}/namespace-cleanup.sh"
...
assert_exit_code() {
  ...
  local actual=0
  "$@" >/tmp/namespace-cleanup-test.out 2>&1 || actual=$?
  if [ "$actual" -eq "$expected" ]; then
    echo "ok   - $desc"
    pass=$((pass + 1))
  else
    echo "FAIL - $desc (expected exit $expected, got $actual)"
    cat /tmp/namespace-cleanup-test.out
    fail=$((fail + 1))
  fi
}
...
assert_exit_code "no namespace target exits non-zero" 1 "$SCRIPT" --fix
...
assert_output_contains "usage mentions --yes" "--yes" "$SCRIPT" --help
...
if command -v kubectl >/dev/null 2>&1 && timeout 5 kubectl cluster-info >/dev/null 2>&1; then
  assert_exit_code "report-only run against default namespace succeeds" 0 "$SCRIPT" -n default
else
